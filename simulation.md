...
## Overview / Objectives
- **Scenario:** Single end-to-end incident involving: SSH brute-force > successful compromise > post-compromise interaction.
- **Objectives:**
...
- It allows for the detection of an **account compromise** such as a successful login from an unusual location or time.
- It can help analysts link events and give them a deeper understanding of an attack. For example:
  - A lone successful SSH login from an unusual location may mean a data leak which includes passwords.
  - A successful SSH login after many brute-force attempts likely means a brute-force attack and the successful discovery of the password.
...
- In the ingested JSON logs, we can clearly see failed brute-force attempts. The username and passwords are displayed in the form **[username/password]**
...
- Cowrie records attacker terminal sessions, producing a replayable recording of the commands the attacker executes inside the honeypot, which is valuable for threat-intelligence and behavioural analysis.
...
- You may be thinking "why are there so many errors", this is because I haven't properly configured the honeypot to use certain binaries or to have certain files therefore many commands won't work.
...
- For efficiency create an automation rule to block suspicious IPs and create tickets for analysis.
